...
int
crossfade_chunk_s16(void *sample_buffer_from, int fadeoffset, void *sample_buffer_to, void *faded_sample_buffer, int sample_start_number, int samples_in_chunk, int total_samples) {
...
*/
	...
	for (n = 0; n < 2; n++) {	
		...
		/*	if (i*number_of_channels + j == 515)	{
			...
			faded_samples[i*number_of_channels + j] = ((((samples_from[fadeoffset + i*number_of_channels + j] * current_fade_amount[0][j]) ) + ((samples_to[i*number_of_channels + j] * current_fade_amount[1][j]) )) );
