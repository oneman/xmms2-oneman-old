...
static gboolean xmms_oggflac_plugin_setup (xmms_xform_plugin_t *xform_plugin);
static gint xmms_oggflac_read (xmms_xform_t *xform, xmms_sample_t *buf, gint len,
                            xmms_error_t *err);
static gboolean xmms_oggflac_init (xmms_xform_t *xform);
static void xmms_oggflac_destroy (xmms_xform_t *xform);
static gint64 xmms_oggflac_seek (xmms_xform_t *xform, gint64 samples, xmms_xform_seek_mode_t whence, xmms_error_t *err);

/*
 * Plugin header
 */

XMMS_XFORM_PLUGIN ("oggflac",
                   "ogg FLAC Decoder", XMMS_VERSION,
                   "Free Lossless Audio Codec in ogg decoder",
                   xmms_oggflac_plugin_setup);

static gboolean
xmms_oggflac_plugin_setup (xmms_xform_plugin_t *xform_plugin)
{
	xmms_xform_methods_t methods;

	XMMS_XFORM_METHODS_INIT (methods);

	methods.init = xmms_oggflac_init;
	methods.destroy = xmms_oggflac_destroy;
	methods.read = xmms_oggflac_read;
	methods.seek = xmms_oggflac_seek;
...
oggflac_callback_read (const FLAC__StreamDecoder *flacdecoder,
                    FLAC__byte buffer[],
...
oggflac_callback_write (const FLAC__StreamDecoder *flacdecoder,
...
oggflac_callback_tell (const FLAC__StreamDecoder *flacdecoder,
...
oggflac_callback_seek (const FLAC__StreamDecoder *flacdecoder,
                    FLAC__uint64 offset, void *client_data)
{
	xmms_error_t err;
	xmms_xform_t *xform = (xmms_xform_t *) client_data;
...
oggflac_callback_length (const FLAC__StreamDecoder *flacdecoder,
...
static void
oggflac_callback_metadata (const FLAC__StreamDecoder *flacdecoder,
...
static FLAC__bool
oggflac_callback_eof (const FLAC__StreamDecoder *flacdecoder,
                   void *client_data)
{
	xmms_xform_t *xform = (xmms_xform_t *) client_data;
...
static void
oggflac_callback_error (const FLAC__StreamDecoder *flacdecoder,
...
static gboolean
xmms_oggflac_init (xmms_xform_t *xform)
{
	xmms_flac_data_t *data;
	...
	init_status =
		FLAC__stream_decoder_init_ogg_stream (data->flacdecoder,
		                                  oggflac_callback_read,
		                                  NULL,
		                                  NULL,
		                                  NULL,
		                                  oggflac_callback_eof,
		                                  oggflac_callback_write,
		                                  oggflac_callback_metadata,
		                                  oggflac_callback_error,
		                                  xform);
...
static gint
xmms_oggflac_read (xmms_xform_t *xform, xmms_sample_t *buf, gint len,
                xmms_error_t *err)
{
...
static gint64
xmms_oggflac_seek (xmms_xform_t *xform, gint64 samples,
...
static void
xmms_oggflac_destroy (xmms_xform_t *xform)
{
	xmms_flac_data_t *data;
